...
import {
  withProjectAccess as newProjectAccess,
  errorHandler,
  notFoundHandler,
} from "./middlewares";
...
// Use new project access middleware but maintain backward compatibility
const withProjectAccess = (requiredRole?: 'freelancer' | 'client') => {
  if (requiredRole) {
    return newProjectAccess(requiredRole);
  }
  
  // Legacy compatibility for routes without specific role requirements
  return async (req: any, res: any, next: any) => {
    try {
      ...
      if (shareToken) {
        ...
        return newProjectAccess('client')(req, res, next);
      } else if (projectId && req.isAuthenticated?.()) {
        // Use freelancer middleware
        return newProjectAccess('freelancer')(req, res, next);
      }
